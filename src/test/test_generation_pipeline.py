...
from src.test.queries import generation_pipeline_test_queries
...
def test_generation_pipeline():
    ...
    # Run queries                                                           #
    # ------------------------------------------------------------------ #
    for i, query in enumerate(generation_pipeline_test_queries, start=1):
        retrieved_docs = retriever.retrieve(query)
        prompt         = prompt_builder.build(query=query, retrieved_docs=retrieved_docs)
        answer         = generator.generate(prompt=prompt)
        response       = formatter.format(answer=answer, retrieved_docs=retrieved_docs)

        print("=" * 80)
        print(f"Test   #{i}")
        print(f"Query  : {query}")
        print(f"Chunks : {len(retrieved_docs)} retrieved")
        print(f"\nAnswer :\n{response['answer']}")
    ...
    print("=" * 80)
    print("Generation pipeline test complete.")
